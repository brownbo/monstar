...
module.exports = app => {
    class EvalController extends app.Controller {
        constructor(ctx) {
            super(ctx, ctx.service.evaluateGift);
        }
        async update (){
        ...
        async exportExcel() {
            ...
            const _data = data.map(item => [
                item.gift && item.gift.name || '无数据', // 接车操作员
                item.gift && item.gift.brand || '无数据', // 入库员
            ...
            // 加上字段名
            _data.unshift(['礼品名称', '礼品品牌', '评价人', '评价时间', '评价内容','回复人',
                '回复时间','回复内容'
            ]);
