import * as service from '../services/gift_type'
...
export default modelExtend(pageModel, {
  ...
  effects: {
    ...
    * update({ payload }, { select, call, put }) {
      const id = yield select(({ gift_type }) => gift_type.currentItem.id);
      const opts = {
        id,
        ...payload
      }
    ...
    * uploadCropImg({ payload }, { select, call, put }) {
      const fileName = yield select(({ gift_type }) => gift_type.fileName);
      const fileList = yield select(({ gift_type }) => gift_type.fileList);
      const oldFile = yield select(({gift_type})=>gift_type.oldFile);
