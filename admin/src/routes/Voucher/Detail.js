...
class VoucherDetail extends React.Component {
  ...
  handClick = () => {
    const { currentItem } = this.props.voucher;
    ...
    this.props.form.validateFields((errors) => {
      ...
      const fileList = this.props.voucher.fileList;
      ...
      data = {
        voucher: data,
        activity: data.is_active ? activeObj : (this.props.voucher.modalType === 'create' ? {} : { enabled: 0 })
      }
      this.props.dispatch({
        type: `${this.api_name}/${this.props.voucher.modalType}`,
        payload: {
          ...data
        }
      });
  ...
  handleChange = ({ file, fileList }) => {
    if (file.status === 'removed') {
      const fileList = this.props.voucher.fileList.filter(item => item.status !== 'removed');
  ...
  showCropImg = (data, file, name) => {
    const fileList = this.props.voucher.fileList.concat([file]);
    this.props.dispatch({
      type: `${this.api_name}/updateState`,
      payload: {
        editorImg: data,
  ...
  render() {
    ...
    const loading_create = this.props.loading.effects['voucher/create'];
    const loading_update = this.props.loading.effects['voucher/update'];
    //TZ时间moment转换有问题，先new Date. 先隐藏有效时间
    /*currentItem.active_start_time = new Date(currentItem.active_start_time);
    currentItem.active_end_time = new Date(currentItem.active_end_time);*/
    ...
    const cropImgProps = {
      cropSize:  1 / 1, //裁剪比例
      editorImg: this.props.voucher.editorImg,
      visible: this.props.voucher.cropModalVisible,
      maskClosable: false,
    ...
    return (
      <div>
          <Form className="detail_form marginTop10" layout="horizontal">
            <Row gutter={24}>
              <Col span={12}>
                <FormItem label="代金券名称" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('name', {
                    initialValue: currentItem.name,
                    rules: [
                      formRules.require,formRules.whitespace,
                    ],
                  })(<Input placeholder="代金券名称"/>)}
                </FormItem>
              </Col>
              <Col span={12}>
              ...
              <Col span={12}>
                <FormItem label="面值（元）" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('value', {
                    initialValue: Func.price(currentItem.value),
                    rules: [
                      formRules.require,
                    ],
                  })(<InputNumber min={0} className="allwidth" placeholder="面值"/>)}
                </FormItem>
              </Col>
              
              {/* <Col span={12}>
                <FormItem label="有效开始时间" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('active_start_time', {
                    initialValue: currentItem.active_start_time?moment(currentItem.active_start_time, 'YYYY-MM-DD HH:mm:ss'):'',
                    rules: [
                      {
                        required:  true,
                      },
              ...
              <Col span={12}>
                <FormItem label="有效结束时间" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('active_end_time', {
                    initialValue:currentItem.active_end_time? moment( currentItem.active_end_time , 'YYYY-MM-DD HH:mm:ss'):'',
                    rules: [
                      {
                        required:  true,
                      },
              ...
              <Col span={12}>
                <FormItem label="支付方式" hasFeedback {...formItemLayout}>
                  ...
                  })(<Radio.Group onChange={e=>{this.setState({pay_type:e.target.value})}}>
              ...
              <Col span={12}>
                <FormItem label="金额支付（元）" hasFeedback {...formItemLayout}>
...
export default connect(({ voucher, shop, loading }) => ({ voucher, shop, loading }))(Form.create()(VoucherDetail))